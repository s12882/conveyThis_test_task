...
- [x] **TASK.md update (2026-09-16): "Ability to safely increase size file limit."** Found the limit was only *really* enforced by 2 of the 3 layers that matter — see below.
...
**"Safely increase the upload limit" (2026-09-16).** User flagged a new `TASK.md` line and asked directly: is bumping nginx's `client_max_body_size` (which they'd already raised to 100M) enough? Checked `php -i` in the running `app` container: `Loaded Configuration File => (none)` — this image had **no `php.ini` at all**, running on PHP's compile-time defaults: `upload_max_filesize=2M`, `post_max_size=8M`. So no — nginx alone doesn't help; PHP itself silently caps/empties uploads over 2-8MB *before Laravel's validation ever runs*, regardless of what nginx allows through. This had been true the entire session and invisible to the test suite, since `UploadedFile::fake()` bypasses real HTTP multipart parsing and never exercises PHP's actual ini enforcement — only a real upload or checking `php -i` directly reveals it.
- Added `docker/php/uploads.ini` (`upload_max_filesize=100M`, `post_max_size=105M`, `memory_limit=256M` — matched to nginx's 100M with headroom), copied into the image via the Dockerfile. Also bumped `fastcgi_read_timeout`/`fastcgi_send_timeout` in `docker/nginx/default.conf` to 300s (nginx's 60s default is easy for a large upload to exceed on a slow connection, which would otherwise fail on an unrelated timeout rather than the size limit — directly relevant to "safely" increasing the cap).
- Chose static, generous infra values over a fully env-var-templated nginx+php.ini system (the other option offered) — simpler, no new entrypoint-script machinery, at the cost of needing two file edits + a rebuild instead of one `.env` change when the limit needs to move again.
- **Verified with real uploads, not just config inspection**: a genuine ~5MB PDF (previously would have been silently rejected/truncated by PHP's old 2M cap) now uploads successfully end-to-end (201, correct `size_bytes`). A genuine ~12MB PDF (over the app's own 10MB `FILE_MAX_SIZE_KB` limit, but under the new 100M infra caps) still gets cleanly rejected with the app's own accurate "File size must not exceed 10 MB" message — confirming the three layers (nginx / PHP / app) now work together correctly rather than the smallest one silently winning. Also confirmed nginx's stale-upstream self-heal (from the earlier M6 fix) still works after recreating `app` for this change. Full suite: 31/31.
- Documented the three-layer requirement and how to move all of them together in `README.md`'s Configuration section, since this is exactly the kind of thing that's easy to half-fix (bump one layer, assume it's done) without realizing the others are still capping silently.

The other two additions in this `TASK.md` revision ("A page to upload a file", "Pagination & sorting of uploaded files") are already fully satisfied by the existing M2/M3 work — no action needed for those.
...
- [x] Cleanup, error handling for edge cases (upload during low disk space, RabbitMQ temporarily down, etc.) — see below

**Edge-case hardening (2026-09-16), the final M6 item.** Re-read `FileDeletionService`/`FileUploadController` looking specifically for the two cases the milestone names, plus whatever else the code-read turned up:
- **`FileDeletionService::delete()` had two real gaps.** (1) `File::find($id)` returning `null` (a real race: e.g. the TTL safety-net reaper reads a batch of expired IDs, and a user manually deletes one before the reaper's loop reaches it) would fatal on `$file->deleteWithReason()`, and the `catch` block's own `Log::error("...{$file->original_name}...")` would then *also* fatal on the still-null `$file` — a double-fault, second exception uncaught. (2) A notification-publish failure (RabbitMQ down) was wrapped in the *same* try/catch as the actual file deletion, so `delete()` returned `false` even when the file really had been deleted — the caller (e.g. `FileController::destroy()`) would report failure for an operation that actually succeeded. Fixed by restructuring into three phases: find-or-return-false-gracefully, delete (own try/catch, returns false only if *this* fails), then notify (best-effort — a failure here is logged but no longer flips the return value, since the file's already gone).
- **`FileUploadController::store()` didn't handle storage failures at all.** Laravel's Flysystem-backed disks throw rather than returning `false` on a write failure (e.g. a full disk), so an uncaught exception would surface as a raw 500. Wrapped `$uploaded->store('uploads')` in a try/catch, logging the failure and returning a clean `503` instead.
- **Tests:** `tests/Feature/FileDeletionServiceTest.php` (missing file → `false`, no crash; notification failure → deletion still succeeds, verified via a throwing stub bound over `AMPQService`) and a new case in `FileUploadTest.php` (storage failure → clean `503`, verified by binding a Mockery-mocked `Filesystem\Factory` that throws on `putFileAs`, simulating a real Flysystem write failure without needing to actually fill a disk).
- **Also verified live against the real broker, not just the mock:** stopped the actual `rabbitmq` container, ran a real deletion via `FileDeletionService` — `delete()` still returned `true`, the file was still correctly soft-deleted and physically removed (took ~4s, mostly the TCP connection attempt timing out before giving up — a latency cost worth knowing about but not fixed further). Confirmed via `storage/logs/laravel.log` that the failure was logged clearly rather than silently swallowed. Restarted `rabbitmq` and `rabbitmq-consumer` afterward.
- **Noted, not changed further:** when `AMQPStreamConnection` construction itself fails (RabbitMQ unreachable at connect time, not mid-operation), `AMPQService`'s constructor catches it internally and logs "Error establishing AMPQ stream", but then `publishFileDeletion()` still throws the less-informative "must not be accessed before initialization" error when it tries to use the never-set connection — this new fix's broad `catch (Throwable $e)` around the publish call correctly absorbs *either* failure shape and produces the same safe outcome (deletion succeeds, notification failure logged), so it's not a functional bug, just a slightly confusing secondary log message chain. Left as-is — flagged in case it's worth cleaning up later.

Full suite: 31/31 passing.

**M6 complete — all milestones (M0–M6) done.** The app implements everything in `TASK.md`: async PDF/DOCX upload with validation (MIME/size/filename-encoding/structural-integrity) and virus/macro scanning, a paginated+sortable file-management page with manual delete, 24h TTL with both a delayed-job path and a scheduled safety-net reaper, and a RabbitMQ-driven deletion-notification pipeline (logged email, per the task's requirements) — all containerized, all verified against the real running stack rather than mocks throughout.

**`AMPQService`/`ConsumeFileDeletions` reviewed after the user's own refactor (2026-09-16).** User moved consume logic into `AMPQService::queryConsume()` for reuse, and asked "Is the Monolog is the best replacement for stdout" after losing the command's `$this->info()` output — confirmed a dedicated Monolog channel (`StreamHandler` → `php://stdout`) over threading `OutputInterface` into the service (would couple a plain service to the console layer) as the best approach here. User implemented it (`consumer_stdout` channel in `config/logging.php`, using `'with'` rather than the more commonly-seen `'handler_with'` key — verified both are actually valid by reading `LogManager::createMonologDriver()` directly, which merges `config['with']` then `config['handler_with']` on top). Review caught one real, severe regression from the refactor: `queryConsume(..., int $limit)` was non-nullable, but the command passes `null` whenever `--limit` is omitted — i.e. every normal/production run — reproduced live as a fatal `TypeError` on the exact invocation `rabbitmq-consumer` uses. User fixed it to `?int $limit = null`, and moved the "Listening on..." message inside `queryConsume()` before the consume loop (previously printed after — meaningless in production since that call never returns, backwards in test mode). Re-verified live: real `TypeError` gone, `docker logs conveythis-rabbitmq-consumer` now shows both the "Listening on" and "Notified..." lines via the new channel exactly as the README describes. Full suite: 28/28. Two minor items flagged but left as-is per the user ("consider this fixed"): the failure path in `handleMessage()`'s catch block still logs to the default channel rather than `consumer_stdout` (so failures won't show in `docker logs`, only successes), and `ConsumeFileDeletions.php` has several now-unused imports left over from before the logic moved into the service.
